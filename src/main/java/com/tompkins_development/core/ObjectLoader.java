...
import com.tompkins_development.core.utils.Utils;
...
public class ObjectLoader {
    ...
    public Model loadOBJModel(String fileName) {
        ...
        float[] texCoorArr = new float[vertices.size() * 2];
        float[] normalArr = new float[vertices.size() * 3];

        for(Vector3i face : faces) {
            processVertex(face.x, face.y, face.z, textures, normals, indices, texCoorArr, normalArr);
        }

        int[] indicesArr = indices.stream().mapToInt((Integer v) -> v).toArray();

        return loadModel(verticesArr, texCoorArr, normalArr, indicesArr);
    }
    ...
    public Model loadModel(float[] vertices, float[] textureCoords, float[] normals, int[] indices) {
        ...
        storeDateInAttribList(0, 3, vertices);
        storeDateInAttribList(1, 2, textureCoords);
        storeDateInAttribList(2, 3, normals);
        unbind();
    ...
    private void storeDateInAttribList(int attribNo, int vertexCount, float[] data) {
        int vbo = GL15.glGenBuffers();
        vbos.add(vbo);
